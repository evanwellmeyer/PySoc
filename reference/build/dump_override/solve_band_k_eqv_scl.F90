...
! appropriate weighted terms for the minor gases.
  CALL dbg_r(dbg_name('k_eqv', i_band, -1), [n_profile, n_layer], PACK(k_eqv(1:n_profile,1:n_layer),.TRUE.))
  CALL dbg_r(dbg_name('k_grey', i_band, -1), [n_profile, n_layer], PACK(k_grey(1:n_profile,1:n_layer),.TRUE.))
  CALL dbg_r(dbg_name('k_grey_tot_final', i_band, -1), [n_profile, n_layer], PACK(ss_prop%k_grey_tot_clr(1:n_profile,1:n_layer),.TRUE.))
  IF (isolir == ip_solar) CALL dbg_r(dbg_name('adjust_solar_ke', i_band, -1), [n_profile, n_layer], PACK(adjust_solar_ke(1:n_profile,1:n_layer),.TRUE.))
...
!   Increment the fluxes within the band.
    CALL dbg_r(dbg_name('flux_total', i_band, iex), [n_profile, 2*n_layer+2], PACK(flux_total_part(1:n_profile,1:2*n_layer+2),.TRUE.))
    IF (isolir == ip_solar) CALL dbg_r(dbg_name('flux_direct', i_band, iex), [n_profile, n_layer+1], PACK(flux_direct_part(1:n_profile,0:n_layer),.TRUE.))
