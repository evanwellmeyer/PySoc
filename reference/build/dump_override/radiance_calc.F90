...
! Calculate temperature interpolation factors for gas k-terms
  IF (ALLOCATED(spectrum%gas%p_lookup)) THEN
    CALL dbg_r('fac00', [atm%n_profile, atm%n_layer], PACK(fac00(1:atm%n_profile,1:atm%n_layer),.TRUE.))
    CALL dbg_r('fac01', [atm%n_profile, atm%n_layer], PACK(fac01(1:atm%n_profile,1:atm%n_layer),.TRUE.))
    CALL dbg_r('fac10', [atm%n_profile, atm%n_layer], PACK(fac10(1:atm%n_profile,1:atm%n_layer),.TRUE.))
    CALL dbg_r('fac11', [atm%n_profile, atm%n_layer], PACK(fac11(1:atm%n_profile,1:atm%n_layer),.TRUE.))
    CALL dbg_i('jp', [atm%n_profile, atm%n_layer], PACK(jp(1:atm%n_profile,1:atm%n_layer),.TRUE.))
    CALL dbg_i('jt', [atm%n_profile, atm%n_layer], PACK(jt(1:atm%n_profile,1:atm%n_layer),.TRUE.))
    CALL dbg_i('jtt', [atm%n_profile, atm%n_layer], PACK(jtt(1:atm%n_profile,1:atm%n_layer),.TRUE.))
  END IF
  n_gas_lk = 0
  i_gas_lk(:) = 0
...
!   Preliminary calculations for source terms:
    CALL dbg_r(dbg_name('k_grey_tot_clr', i_band, -1), [atm%n_profile, atm%n_layer], PACK(ss_prop%k_grey_tot_clr(1:atm%n_profile,1:atm%n_layer),.TRUE.))
    CALL dbg_r(dbg_name('k_ext_scat_clr', i_band, -1), [atm%n_profile, atm%n_layer], PACK(ss_prop%k_ext_scat_clr(1:atm%n_profile,1:atm%n_layer),.TRUE.))
    CALL dbg_r(dbg_name('phase_fnc_clr', i_band, -1), [atm%n_profile, atm%n_layer], PACK(ss_prop%phase_fnc_clr(1:atm%n_profile,1:atm%n_layer,1),.TRUE.))
    CALL dbg_r(dbg_name('forward_scatter_clr', i_band, -1), [atm%n_profile, atm%n_layer], PACK(ss_prop%forward_scatter_clr(1:atm%n_profile,1:atm%n_layer),.TRUE.))
    IF (control%l_continuum .AND. n_continuum > 0) CALL dbg_r(dbg_name('amount_continuum', i_band, -1), [atm%n_profile, atm%n_layer, n_continuum], PACK(amount_continuum(1:atm%n_profile,1:atm%n_layer,1:n_continuum),.TRUE.))
...
!   Allocate space for the Planckian emission fields
    ...
    CALL dbg_i(dbg_name('n_abs_esft', i_band, -1), [n_abs], n_abs_esft(1:n_abs))
    CALL dbg_i(dbg_name('index_abs', i_band, -1), [n_abs], index_abs(1:n_abs))
    CALL dbg_r(dbg_name('w_abs_esft', i_band, -1), [nd_k_term, n_abs], PACK(w_abs_esft(1:nd_k_term,1:n_abs),.TRUE.))
    CALL dbg_r(dbg_name('k_abs_layer', i_band, -1), [atm%n_profile, atm%n_layer, nd_k_term, n_abs], PACK(k_abs_layer(1:atm%n_profile,1:atm%n_layer,1:nd_k_term,1:n_abs),.TRUE.))
...
!   the overlap assumed:
    ...
    IF (control%isolir == ip_infra_red) THEN
      CALL dbg_r(dbg_name('planck_flux', i_band, -1), [atm%n_profile, atm%n_layer+1], PACK(planck%flux(1:atm%n_profile,0:atm%n_layer),.TRUE.))
      CALL dbg_r(dbg_name('planck_diff', i_band, -1), [atm%n_profile, atm%n_layer], PACK(planck%diff(1:atm%n_profile,1:atm%n_layer),.TRUE.))
      CALL dbg_r(dbg_name('planck_diff_2', i_band, -1), [atm%n_profile, atm%n_layer], PACK(planck%diff_2(1:atm%n_profile,1:atm%n_layer),.TRUE.))
      CALL dbg_r(dbg_name('planck_flux_ground', i_band, -1), [atm%n_profile], planck%flux_ground(1:atm%n_profile))
    END IF
