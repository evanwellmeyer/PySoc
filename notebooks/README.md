...
* The code in every cell is hidden behind a `#@title` (Colab's form view); students click *Show code* to read it.
  In notebook 2 the values to change are Colab form fields (`#@param` sliders, tick boxes and menus) next to
  each cell's title: students change them, rerun the cell and write their descriptions in a *Your notes* cell.
  Outside Colab the form fields are ordinary assignments at the top of each cell.
